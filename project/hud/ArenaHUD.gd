...
func _ready():
	...
	round_number.texture = ROUND_TEXTURES[Global.round_number - 1]
	
	for i in range(player_scores.size()):
		player_scores[i].set_score(Global.scores[i])
...
func show_round():
	...
	if Global.winner == -1:
		...
		player_score = player_scores[Global.winner]
...
func win_animation():
	...
	round_winner.rect_position = WINNER_POS[Global.winner]
